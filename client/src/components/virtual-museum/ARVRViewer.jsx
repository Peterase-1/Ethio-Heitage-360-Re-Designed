...
const ARVRViewer = ({ artifact, onClose }) => {
    ...
    return (
      <div className="w-full h-full bg-gradient-to-br from-gray-900 via-gray-800 to-black flex items-center justify-center relative overflow-hidden">
        ...
        <div className="absolute inset-0">
          <div className="absolute w-96 h-96 bg-primary/5 rounded-full blur-3xl animate-pulse" style={{ top: '10%', left: '10%' }}></div>
          <div className="absolute w-64 h-64 bg-secondary/5 rounded-full blur-2xl animate-pulse delay-1000" style={{ bottom: '20%', right: '15%' }}></div>
        </div>
        ...
        <div className="relative z-10">
          ...
          >
            <div className="bg-gradient-to-br from-card to-muted rounded-3xl shadow-2xl overflow-hidden border-4 border-white/20 backdrop-blur-sm">
              <div className="relative group">
                ...
                <div className="absolute top-1/4 left-1/3 w-4 h-4 bg-primary rounded-full animate-ping cursor-pointer" title="Click for details"></div>
                <div className="absolute bottom-1/3 right-1/4 w-4 h-4 bg-secondary rounded-full animate-ping delay-500 cursor-pointer" title="Historical context"></div>
              ...
              {/* Enhanced info panel */}
              <div className="p-6 bg-gradient-to-r from-background to-muted">
                <h2 className="text-2xl font-bold text-foreground mb-3">{artifact.name}</h2>
                <p className="text-muted-foreground mb-4 leading-relaxed">{artifact.description}</p>

                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="bg-card/60 rounded-lg p-3">
                    <span className="font-semibold text-secondary-foreground">Origin:</span>
                    <br />
                    <span className="text-muted-foreground">{artifact.origin}</span>
                  </div>
                  <div className="bg-card/60 rounded-lg p-3">
                    <span className="font-semibold text-secondary-foreground">Period:</span>
                    <br />
                    <span className="text-muted-foreground">{artifact.period}</span>
                  </div>
                  <div className="bg-card/60 rounded-lg p-3">
                    <span className="font-semibold text-secondary-foreground">Category:</span>
                    <br />
                    <span className="text-muted-foreground">{artifact.category}</span>
                  </div>
                  <div className="bg-card/60 rounded-lg p-3">
                    <span className="font-semibold text-secondary-foreground">Museum:</span>
                    <br />
                    <span className="text-muted-foreground text-xs">{artifact.museum}</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <div className={`fixed inset-0 bg-black z-50 ${isFullscreen ? '' : 'p-8'}`}>
      ...
      <div className="absolute top-4 left-4 right-4 z-20 flex justify-between items-center">
        <div className="flex space-x-2">
          <button
            ...
            className={`px-4 py-2 rounded-lg flex items-center space-x-2 transition-all ${viewMode === '3d'
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg flex items-center space-x-2 transition-all ${viewMode === 'ar'
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg flex items-center space-x-2 transition-all ${viewMode === 'vr'
        ...
        <div className="flex space-x-2">
          <button
            ...
            className="p-2 bg-card/90 text-muted-foreground rounded-lg hover:bg-card transition-colors"
            title="Zoom Out"
          >
            <ZoomOut size={18} />
          </button>
          ...
          <button
            ...
            className={`px-3 py-2 rounded-lg flex items-center space-x-1 transition-all ${showInfo
          ...
          <button
            onClick={resetView}
            className="px-3 py-2 bg-card/90 text-muted-foreground rounded-lg hover:bg-card transition-colors flex items-center space-x-1"
          >
            <RotateCcw size={18} />
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 bg-destructive text-destructive-foreground rounded-lg hover:bg-destructive/90 transition-colors flex items-center space-x-1"
          >
            <X size={18} />
      ...
      {/* Info Panel */}
      {showInfo && (
        <div className="absolute top-20 right-4 w-80 bg-background/95 backdrop-blur-sm rounded-xl shadow-2xl p-6 z-20 border border-border">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-bold text-foreground">Virtual Museum Guide</h3>
            <button
          ...
          <div className="space-y-4">
            <div>
              <h4 className="font-semibold text-muted-foreground mb-2">Controls:</h4>
            ...
            <div>
              <h4 className="font-semibold text-muted-foreground mb-2">View Modes:</h4>
              <ul className="text-sm text-muted-foreground/80 space-y-1">
            ...
            <div className="pt-2 border-t border-border">
              <h4 className="font-semibold text-muted-foreground mb-2">Artifact Details:</h4>
      ...
      <div className="absolute bottom-4 right-4 text-white">
        <div className="bg-black/50 backdrop-blur-sm px-4 py-2 rounded-lg">
          <span className="text-sm font-medium">
            Mode: <span className="text-primary">{viewMode.toUpperCase()}</span>
          </span>
        </div>
      </div>
    </div>
